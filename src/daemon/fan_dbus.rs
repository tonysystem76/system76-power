...
pub struct FanDbus {
    // Shared FanDaemon instance used by both DBus handlers and the main loop
    fan: Arc<Mutex<FanDaemon>>,
}

impl FanDbus {
    pub fn new(shared_fan: Arc<Mutex<FanDaemon>>) -> Self {
        log::info!("Creating FanDbus with shared FanDaemon instance");
        Self { fan: shared_fan }
    }
